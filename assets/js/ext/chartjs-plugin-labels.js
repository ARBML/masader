...
(function () {
        ...
        this.options = Object.assign(
            {
                ...
                fontStyle: chartOptions.font
                    ? chartOptions.font.style
                    : 'normal',
    ...
    Label.prototype.renderArcLabel = function (label, renderInfo) {
        ...
        if (typeof label === 'string') {
            ...
            for (let j = 0; j < lines.length; ++j) {
                const line = lines[j];
                const y =
                    (lines.length - 1 - j) * -this.options.fontSize + offset;
                ctx.save();
            ...
            ctx.rotate(
                (view.startAngle + Math.PI / 2 + renderInfo.endAngle) / 2
            );
            ctx.translate(0, -1 * radius);
        ...
        if (
            ...
            if (!this.barTotal[index]) {
                ...
                for (let i = 0; i < this.chart.data.datasets.length; ++i) {
                    this.barTotal[index] +=
                        this.chart.data.datasets[i].data[index];
                }
            }
        ...
        if (!this.options.showActualPercentages) {
            ...
            if (this.totalPercentage > 100) {
                ...
                percentage = parseFloat(
                    percentage.toFixed(this.options.precision)
                );
            }
            if (this.chart.config.type === 'bar') {
    ...
    Label.prototype.getBaseRenderInfo = function (element, label) {
        if (
            ...
            if (this.options.position === 'border') {
                rangeFromCentre =
                    (view.outerRadius - innerRadius) / 2 + innerRadius;
            ...
            if (this.options.position === 'outside') {
                const offset =
                    this.options.textMargin +
                    this.measureLabel(label).width / 2;
    ...
    Label.prototype.drawable = function (element, label, renderInfo) {
        if (this.options.overlap) {
            ...
            return (
                renderInfo.endAngle - renderInfo.startAngle <=
                renderInfo.totalAngle
            );
        } else {
            const mertrics = this.measureLabel(label),
        ...
        beforeDatasetsUpdate: function (chart, args, options) {
            ...
            for (let i = 0; i < count; ++i) {
                ...
                if (label.options.position === 'outside') {
                    someOutside = true;
                    const padding =
                        label.options.fontSize * 1.5 +
                        label.options.outsidePadding;
