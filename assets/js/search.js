    ...
    $('#pagination').pagination({
        ...
        dataSource: (cb) => {
            ...
            request
                ...
                .then((data) => MasaderFuzzy.filterDatasets(data, searchName))
                .then(cb);
        },
        callback: (data) => {
            let html = '';
            ...
            for (let element of data) {
                ...
                for (let attribute in element) {
                    ...
                    html += `<span class='truncate'>${setAttributes(
                        attribute,
                        element
                    )}</span>`;
                    html += '</div>';
    ...
    request
        ...
        .then((object) => {
            for (let k in object) {
                ...
                if (shove[k])
                    object[k] = object[k].sort((e) =>
                        shove[k].includes(e) ? -1 : 1
                    );
                ...
                through.addEventListener('keyup', (event) => {
                    ...
                    if (value.length > 0)
                        $(expander).parent().addClass('hidden');
                    else $(expander).parent().removeClass('hidden');

                    const allLabels = elements.map(
                        (el) => $(el).children('input')[0].value
                    );
                    ...
                    elements.forEach((el, index) => {
                        ...
                        const isMatching =
                            value.length === 0 || matchingLabels.has(label);
